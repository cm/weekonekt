        ...
        (let* (
          ...
          (req10 (cowboy_req:compact req9)))
            (case (route input state)
              ...
              ((tuple 'redirect url 'cookie name value path expires)(redirect req10 state url name value path expires))
              (json (reply req10 state 200 json))))))))))


(defun redirect (req state url cookie value path expires)
  (let* ((req2 (cowboy_req:set_resp_cookie cookie value 
                 (list 
                    (tuple 'max_age expires)
                    (tuple 'path path)) req)) 
         ((tuple 'ok req3)(cowboy_req:reply 302 (list (tuple #"Location" url)) req2)))
    (tuple 'ok req3 state)))
...
(defun route (i mod)
  ...
  (let ((a (map-get i #"action")))
    (case (call mod 'do a) 
      ...
      ((tuple acl v f)(route 'default a acl v i f))
      ((tuple 'relaxed acl v f)(route 'relaxed a acl v i f)))))


(defun route (profile a acl v i f)
  (case (security profile a acl i)
    ((tuple 'error e)(tuple 'error e))
    ((tuple 'ok app user) 
      (case (kit:val v i )
        ((tuple 'error e)(tuple 'error e))
  ...
  (('default a acl i)
    (case (kit:val '(#(app #"app") #(lang #"lang") #(text #"ua") #(text #"ip")) i)
      ...
      ((tuple 'ok (list app lang ua ip))
        (security a acl i app lang ua ip))))
  (('relaxed a acl i)
    (case (kit:val '(#(text #"ua") #(text #"ip")) i)
      ((tuple 'error e) (tuple 'error e))
      ((tuple 'ok (list ua ip))
        (security a acl i 'undefined 'en ua ip)))))

(defun security (a acl i app lang ua ip )
    ...
    (case token
      ...
      (_ 
        (case acl
          ...
          (_ 
            (users:find_session token app ua ip (lambda (u)
              (users:extend_session token (lambda ()
                (case acl  
                  ('signed_in (tuple 'ok app u))
                  (_ 
                    (case (users:get_valid_acl u (kit:atom2bin acl) )
